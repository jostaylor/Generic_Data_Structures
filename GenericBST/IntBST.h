...
void IntBST::recDelete(TreeNode *node){
  if (node){
    recDelete(node->left);
...
void IntBST::recPrint(TreeNode *node){
  if (node == NULL){
    return;
  }
...
void IntBST::printTree(){
  recPrint(root);
}

TreeNode* IntBST::getMax(){
...
bool IntBST::deleteNode(int value){
...
TreeNode* IntBST::getSuccessor(TreeNode *d){ // d is the node to be deleted
